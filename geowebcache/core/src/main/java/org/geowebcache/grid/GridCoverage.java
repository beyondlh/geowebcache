...
public class GridCoverage {
    ...
    protected long[] getIntersection(long[] rectangle) {
        ...
        long[] ret =
                {
                        //coverage  The extent of the data. {minx,miny,maxx,maxy,zoomlevel}
                        Math.min(Math.max(coverage[0], rectangle[0]), coverage[2]),//minx
                        Math.min(Math.max(coverage[1], rectangle[1]), coverage[3]),//miny
                        Math.min(Math.max(coverage[0], rectangle[2]), coverage[2]),//maxx
                        Math.min(Math.max(coverage[1], rectangle[3]), coverage[3]),//maxy
                        rectangle[4]                                                //level
                };
